...
1. Acesse o [Graph API Explorer da Meta](https://developers.facebook.com/tools/explorer/)
2. Crie uma App em developers.facebook.com (caso não tenha)
3. Em **User or Page** → **Get Page Access Token** → escolha sua Página
...
5. Copie o token gerado e cole na tela de onboarding
...
1. Substitua `better-sqlite3` por `pg` no `package.json`
2. Atualize `lib/db.js` para usar `pg.Pool`
3. Adapte as queries (a maioria é SQL padrão e funciona; ajuste apenas `INSERT OR IGNORE`, `datetime('now')` e tipos `BOOLEAN`)
4. Migre os dados com `pgloader` ou script customizado
